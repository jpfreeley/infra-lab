#!/bin/bash
# Idle auto-stop script — runs every 5 minutes via cron
...
# Cron: */5 * * * * root /usr/local/bin/idle-check.sh
...
# ss with 'state established' already filters — count non-header output lines
CODE_SERVER_CONNECTIONS=$(ss -tn state established '( dport = :8080 or sport = :8080 )' 2>/dev/null | tail -n +2 | wc -l)
...
if [ "$IDLE_MINUTES" -ge "$IDLE_THRESHOLD_MINUTES" ]; then
    logger -t idle-check "No DCV or code-server connections for ${IDLE_MINUTES} minutes. Stopping instance."
