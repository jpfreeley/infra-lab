...
Invoked by EventBridge Scheduler with {"slot": N}. Strategy text, prompts and
numeric limits are read at runtime from a private S3 config prefix, never
...
def _alert(topic, title, message):
    notify.send(topic, title, message, priority=5, tags="rotating_light")


def _load_prompt_docs(store, slot):
    """Read the private strategy, guardrail text and prompts for this slot."""
    docs = []
    for key in (
        "config/STRATEGY.md",
        "config/GUARDRAILS.md",
        "config/prompts/COMMON.md",
    ):
    ...
    listing = store.s3.list_objects_v2(
        Bucket=store.bucket, Prefix=f"config/prompts/slot-{slot}-"
    )
...
def _flatten(client, toolbox, topic, dry_run):
    ...
    _alert(topic, "FLATTEN FAILED", f"Positions still open after retries: {symbols}")
...
def _fallback_report(toolbox, topic, dry_run):
    ...
    notify.send(topic, prefix + "Paper trading report (auto)", "\n".join(lines))
...
def _run(event, context):
    ...
    store = Store(s3, bucket, "dryrun/" if dry_run else "")
    now = datetime.now(ET)
    today = now.date().isoformat()
    topic = _secret(sm, _env("NTFY_SECRET_ARN"))

    if not dry_run:
        control = store.get_json("control/enabled.json", {}, absolute=True) or {}
        in_window = (
    ...
    try:
        params = guardrails.Params.from_dict(
            store.get_json("config/guardrails.json", None, absolute=True)
        )
    except guardrails.ConfigError as exc:
        _alert(topic, "Trading config invalid", str(exc))
        return {"ok": False, "error": str(exc)}

    keys = json.loads(_secret(sm, _env("ALPACA_SECRET_ARN")))
    client = AlpacaClient(keys["key_id"], keys["secret_key"])
    bad = guardrails.paper_check(client.trading_url, client.account())
    if bad:
        _alert(topic, "Paper check failed", bad)
        return {"ok": False, "error": bad}

    if not dry_run and not client.calendar(today):
        return {"ok": True, "skipped": "market closed today"}

    ctx = RunContext(slot=slot, day=today, dry_run=dry_run, ntfy_topic=topic)
    ...
    try:
        ...
        _alert(topic, "Run skipped: lock held", f"slot {slot}: {exc}")
    ...
    try:
        ...
        return _run_agent(store, toolbox, params, ctx, topic, slot, now)
        ...
        _alert(
            topic, f"Trading run failed (slot {slot})", f"{type(exc).__name__}: {exc}"
        )
        return {"ok": False, "error": str(exc)}
    finally:
        if locked:
...
def _run_agent(store, toolbox, params, ctx, topic, slot, now):
    ...
    if result["stopped"] != "finished":
        _alert(topic, f"Run stopped early (slot {slot})", json.dumps(result))
...
def handler(event, context):
    ...
    try:
        ...
        try:
            ...
            _alert(topic, "Trading startup failure", f"{type(exc).__name__}: {exc}")
